...
const TourHighlights = () => {
  ...
  return (
    <section className="2xl:max-w-[1920px] bg-gradient-to-b from-[#B6A28F] to-[#8C6B4A] relative flex flex-col py-16 lg:py-24 overflow-hidden">
      ...
      <div
        ref={scrollRef}
        className="flex gap-6 whitespace-nowrap overflow-x-hidden px-6 cursor-grab touch-pan-x"
      >
        {[...highlights, ...highlights].map((item, index) => (
          <div
            key={index}
            className="flex-shrink-0 w-64 sm:w-72 md:w-80 lg:w-72 p-6 bg-white/10 rounded-3xl backdrop-blur-md text-white flex flex-col items-center gap-4 
            hover:scale-110 hover:bg-white/20 transform transition-all duration-500 animate-fadeUp"
          >
            <div className="mb-2">{item.icon}</div>
            <h3 className="text-xl font-semibold text-white text-center">{item.title}</h3>
            <p className="text-center text-white/80 text-sm break-words">{item.description}</p>
          </div>
        ))}
      </div>
